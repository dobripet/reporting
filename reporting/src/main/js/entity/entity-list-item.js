...
/**
 * Entity component, represent one entity from config
 *
...
export default class EntityListItem extends React.Component {
    ...
    render() {
        ...
        if (this.state.showStatsModal) {
            stats = <EntityStatsModal entity={entity} onClose={this.handleCloseModal}
                                      getEntityRowCount={getEntityRowCount}/>;
        }
        return (
            <div className="list-item">
                ...
                <span onMouseOver={this.handleMouseOver} onMouseLeave={this.handleMouseLeave}
                      onClick={this.handleExpand} className="custom-item cursor-pointer"
                      style={{fontSize: "18px"}}>{entity.name}</span>
                ...
                <button onClick={this.handleAdd} className="custom-item btn btn-primary btn-xs"
                        style={{verticalAlign: "text-bottom"}}>
                    <span className="glyphicon glyphicon-plus-sign cursor-pointer"></span><span className="custom-item">All</span>
                </button>
                {stats}
                {propertyItems}
            </div>
