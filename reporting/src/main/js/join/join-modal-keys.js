...
export default class JoinModalKeys extends React.Component {
    ...
    componentWillReceiveProps(nextProps){
        console.log("keys next props", nextProps);
    ...
    render() {
        let keys = this.state.keys.map((key, i) => <div className="checkbox" key={i}>
                <label><input type="checkbox" value={i} checked={key.selected} onChange={this.onChange}/>{key.localColumnName}->{key.foreignColumnName}</label>
            </div>);
        return (
            <div>
                {keys}
